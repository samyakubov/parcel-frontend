...
function GetTrackBackground (currentYear:number) {
	...
	const startPercentage = ((searchContext.advancedSearchQuery.year_filed_start - min) / (max - min)) * 100
	const endPercentage = ((searchContext.advancedSearchQuery.year_filed_end - min) / (max - min)) * 100
	...
	return (
		<div className="space-y-4">
			...
			<StyledDropdown options={DOCUMENT_TYPES}
				onChange={(e) => searchContext.setAdvancedSearchQuery("document_type", e)}/>

			<div className="space-y-2">
				<label className="block mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">
					Date
					Range: {searchContext.advancedSearchQuery.year_filed_start} - {searchContext.advancedSearchQuery.year_filed_end}
				</label>
				<div className="relative h-2">
					...
					<div className="relative">
						<input
							...
							max={currentYear}
							step={1}
							value={searchContext.advancedSearchQuery.year_filed_end}
							onChange={(e) => {
								const newEnd = Number(e.target.value)
								searchContext.setAdvancedSearchQuery("year_filed_end", Math.max(newEnd, searchContext.advancedSearchQuery.year_filed_start))
							}}
						...
						<input
							...
							max={currentYear}
							step={1}
							value={searchContext.advancedSearchQuery.year_filed_start}
							onChange={(e) => {
								const newStart = Number(e.target.value)
								searchContext.setAdvancedSearchQuery("year_filed_start", Math.min(newStart, searchContext.advancedSearchQuery.year_filed_end))
							}}
			...
			<div>
				...
				<StyledDropdown options={PROPERTY_TYPES} onChange={(e) => {
					searchContext.setAdvancedSearchQuery("property_type", e)
				}}/>
			</div>
